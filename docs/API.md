...
If ranked providers remain but all execute-ready options collapse to zero in the current context, `resolve` now keeps the ranked `providers` list while returning `fallback_chain=[]`, `execute_hint=null`, and `recovery_hint.reason=no_execute_ready_providers` plus machine-readable degraded-provider context like `unavailable_provider_slugs` and `not_execute_ready_provider_slugs`. When this happens under a requested `credential_mode`, `supported_provider_slugs` and `supported_credential_modes` still reflect the broader unfiltered pivot so callers can recover toward another viable rail without another blind search.
