...
| `CI / cli-test` | pytest + black + ruff pass after pin; mypy was 5 `union-attr` in `tester_fleet.py` | Pins CLI dev tools to API versions. Narrow `run`/`summary` dicts so mypy 1.11.2 accepts them. |
