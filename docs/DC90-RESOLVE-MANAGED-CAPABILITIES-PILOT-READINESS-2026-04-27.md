...
- **Next expansion action:** after `0164_emailable_managed_visibility_repair.sql` deploys, rerun Emailable `email.verify`; otherwise the next named amber fixture should be a bounded file/media/document fixture or a consented side-effect fixture. E2B create/status/cleanup lifecycle is now proved, but arbitrary E2B code execution and long-lived sandboxes remain out of scope until a separate spend/cleanup gate exists.
