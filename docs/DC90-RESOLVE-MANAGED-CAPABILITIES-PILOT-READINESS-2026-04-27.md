...
- **Next expansion action:** after `0164_emailable_managed_visibility_repair.sql` deploys, rerun Emailable `email.verify`; otherwise the next named amber fixture should be a bounded Deepgram media transcription/subtitle fixture or a consented side-effect fixture. E2B create/status/cleanup lifecycle and Unstructured `document.parse` are now proved, but arbitrary E2B code execution, long-lived sandboxes, customer documents, and broader document variants remain out of scope until separate fixture/spend/cleanup gates exist.
