...
Emailable `email.verify` was attempted as a green fixture on 2026-04-28, but estimate returned HTTP 503 `provider_not_available` before execution because the hosted managed catalog did not expose `email.verify` / `emailable`. A later recheck after `0164` had been pushed, and the latest `scripts/dc90_managed_fixture_smoke.py --skip-algolia` rerun at `artifacts/dc90-managed-fixture-smoke-20260428T043401Z.json`, still showed no `rhumb_managed` providers on resolve and `provider_not_available` on estimate, so Emailable remains pending until deploy/migration convergence exposes the rows.

Interpretation: the trusted-user pilot can honestly say that Rhumb-managed execution has fresh hosted proof across search-adjacent, embedding, Algolia read-only search/autocomplete/product-search plus one disposable index-write fixture, scraping, IP enrichment, IP lookup aliases, text-generation rails, one short-TTL E2B sandbox lifecycle, one tiny synthetic document-parse fixture, and one tiny Rhumb-generated media transcription fixture. This is still **first-wave proof**, not a universal claim that all managed configs work.
...
- Start with `search.query` only for the first invite unless the user has an explicit need for one of the freshly proven rails (`ai.embed`, `ai.generate_text`, `document.search`, `search.autocomplete`, `ecommerce.search_products`, disposable `search.index`, `scrape.extract`, `data.enrich`, `geo.lookup`, `identity.lookup`, `timezone.get_info`, short-TTL `agent.spawn` / `agent.get_status`, synthetic-file `document.parse`, or tiny-fixture `media.transcribe`).
...
- **Next expansion action:** after `0164_emailable_managed_visibility_repair.sql` deploys, rerun Emailable `email.verify`; otherwise the next named amber fixture should be `video.subtitle` on the same tiny media asset only if the provider/route can return subtitle-shaped output, or a consented side-effect fixture. E2B create/status/cleanup lifecycle, Unstructured `document.parse`, and Deepgram `media.transcribe` are now proved, but arbitrary E2B code execution, long-lived sandboxes, customer documents, broader document variants, and uncontrolled external sends remain out of scope until separate fixture/spend/cleanup gates exist.
