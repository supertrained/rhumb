...
describe("types.contract", () => {
  ...
  it("generated llms, agent capabilities, and MCP metadata stay aligned with the live rail-based execution story", () => {
    ...
    expect(mcpReadme).toContain("Rhumb scores **1,038 services** and exposes **415 capability definitions**");
    expect(mcpReadme).toContain("current governed execution is concentrated in **16 callable providers**");
    ...
    expect(mcpReadme).not.toContain("999 services");
    expect(mcpReadme).not.toContain("435 capability definitions");
    expect(mcpReadme).not.toContain("18 callable providers");
    ...
    expect(mcpServerManifest.version).toBe("2.0.0");
    expect(mcpServerManifest.packages[0].version).toBe("2.0.0");
