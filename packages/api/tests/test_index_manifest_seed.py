...
def test_seed_rows_preserve_evidence_metadata_only_where_available() -> None:
    ...
    rows_without_evidence = [
        row for row in rows.values() if row["route_id"] != search_row["route_id"]
    ]
...
def test_seeded_rows_hydrate_route_facts_and_fail_closed_recommendation_policy() -> None:
    ...
    search_facts = _route_facts_from_row(
        rows["route_search_query_brave_search_api_official_api_v1"]
    )
    assert search_facts["manifest_digest"].startswith("sha256:")
    assert (
        search_facts["evidence_packet_id"]
    ...
    generated = with_recommendation_policy(
        _manifest_from_row(rows["route_calendar_freebusy_generated_adapter_v1"])
    )
    ...
    assert (
        "source_risk_community_unverified_not_default"
        in generated["recommendation_policy"]["reasons"]
    )

    write_route = with_recommendation_policy(
        _manifest_from_row(rows["route_support_ticket_create_zendesk_api_official_api_v1"])
    )
    ...
    cli_route = with_recommendation_policy(
        _manifest_from_row(rows["route_workflow_run_list_github_cli_v1"])
    )
    assert cli_route["recommendation_policy"]["default_recommendable"] is False
    assert (
        "source_risk_community_unverified_not_default"
        in cli_route["recommendation_policy"]["reasons"]
    )
