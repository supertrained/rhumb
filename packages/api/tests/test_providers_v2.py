...
_BRAVE_CANONICAL_ALTERNATE_ALIAS_TEXTY_SERVICE_DETAIL = {
    "slug": "brave-search-api",
...
def _mock_supabase_fetch_with_canonical_texty_callable_provider(query: str):
    if query.startswith("capability_services?"):
        if "capability_id=eq.search.query" in query:
            return [{"service_slug": "brave-search"}]
        if "service_slug=eq.brave-search" in query:
    ...
    if query.startswith("services?"):
        if "slug=eq.brave-search-api" in query:
            return [_BRAVE_CANONICAL_ALTERNATE_ALIAS_TEXTY_SERVICE_DETAIL]
        if "slug=eq.brave-search" in query:
            return []
        if "slug=in." in query and "brave-search" in query:
            return [_BRAVE_CANONICAL_ALTERNATE_ALIAS_TEXTY_SERVICE_DETAIL]
...
class TestListProviders:
    ...
    def test_list_rejects_invalid_capability_filter(self, client):
        with (
            patch("routes.providers_v2._all_direct_provider_mappings", return_value=[]),
            patch("routes.providers_v2.supabase_fetch", new=AsyncMock(return_value=[])) as mock_fetch,
        ):
            resp = client.get("/v2/providers?capability=not.real")
    ...
    def test_list_rejects_blank_capability_filter(self, client):
        with patch("routes.providers_v2.supabase_fetch", new=AsyncMock()) as mock_fetch:
            resp = client.get("/v2/providers?capability=%20%20")
    ...
    def test_list_rejects_invalid_offset_with_canonical_envelope(self, client):
        with patch("routes.providers_v2.supabase_fetch", new=AsyncMock()) as mock_fetch:
            resp = client.get("/v2/providers?offset=-1")
    ...
    def test_list_normalizes_capability_filter_whitespace(self, client):
        with (
            ...
            resp = client.get(f"/v2/providers?capability=%20{_TEST_CAPABILITY}%20&status=listed")

        assert resp.status_code == 200
        data = resp.json()["data"]
        providers_by_id = {provider["id"]: provider for provider in data["providers"]}
        assert list(providers_by_id) == [_TEST_PROVIDER_SLUG]
    ...
    def test_list_canonicalizes_alternate_alias_mentions_in_canonical_provider_metadata_text(self, client):
        with patch(
            "routes.providers_v2.supabase_fetch",
            side_effect=_mock_supabase_fetch_with_canonical_texty_callable_provider,
        ):
            resp = client.get("/v2/providers?capability=search.query")
...
class TestGetProvider:
    ...
    def test_get_provider_defaults_malformed_catalog_credential_modes_field(self, client):
        def mock_fetch(query: str):
            if (
                query.startswith("capability_services?")
                and f"service_slug=eq.{_TEST_PROVIDER_SLUG}" in query
                ...
                return [
                    {
                        **_MOCK_CAPABILITY_MAPPING,
                        "credential_modes": {"mode": "agent_vault"},
                    }
                ]
            return _mock_supabase_fetch(query)
    ...
    def test_get_provider_canonicalizes_alternate_alias_mentions_in_canonical_metadata_text(self, client):
        with patch(
            "routes.providers_v2.supabase_fetch",
            side_effect=_mock_supabase_fetch_with_canonical_texty_callable_provider,
        ):
...
class TestExecuteOnProvider:
    ...
    def test_execute_rejects_invalid_credential_mode_before_reads(self, client):
        with patch("routes.providers_v2._resolve_provider_detail", new_callable=AsyncMock) as mock_detail:
            resp = client.post(
        ...
        error = resp.json()["error"]
        assert error["code"] == "INVALID_PARAMETERS"
        assert error["message"] == "Invalid 'credential_mode' field."
    ...
    @patch("routes.providers_v2.get_receipt_service")
    @patch("routes.providers_v2.supabase_fetch", side_effect=_mock_supabase_fetch)
    def test_execute_normalizes_credential_mode_before_forwarding_and_receipts(
        self,
        mock_fetch,
        mock_receipt_svc,
        mock_budget,
        ...
        mock_receipt = MagicMock()
        mock_receipt.receipt_id = "rcpt_credential_mode_123"
        mock_receipt_svc.return_value.create_receipt = AsyncMock(return_value=mock_receipt)

        mock_execute_response = {
            "data": {
                "execution_id": "exec_credential_mode_123",
                "result": {"text": "Hello"},
                "provider_latency_ms": 100,
                "agent_id": "test_agent",
                "org_id": "test_org",
