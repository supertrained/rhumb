...
CANONICAL_ROW_ALTERNATE_ALIAS_TEXTY_SERVICES = [
    {
        "slug": "brave-search-api",
        "name": "Brave Search API after pdl comparison",
        "category": "search",
    },
    {"slug": "people-data-labs", "name": "People Data Labs", "category": "data"},
]
...
async def test_list_capabilities_rejects_invalid_limit_before_registry_reads(app):
    with patch("routes.capabilities._cached_fetch", new=AsyncMock()) as mock_fetch:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/v1/capabilities?limit=0")
...
@pytest.mark.anyio
async def test_get_capability_canonicalizes_alternate_alias_mentions_in_provider_service_name_for_canonical_rows(app):
    canonical_mapping = [{
        **ALIAS_MAPPINGS[0],
    ...
    async def mock_fetch(path: str):
        ...
        if path.startswith("services?"):
            return CANONICAL_ROW_ALTERNATE_ALIAS_TEXTY_SERVICES
        if path.startswith("bundle_capabilities?"):
            return []
        return []
...
@pytest.mark.anyio
async def test_get_capability_canonicalizes_alternate_alias_mentions_in_notes_for_canonical_rows(app):
    canonical_mapping = [{
        **ALIAS_MAPPINGS[0],
        "service_slug": "brave-search-api",
...
@pytest.mark.anyio
async def test_resolve_capability_ignores_non_string_credential_modes_from_catalog(app):
    async def mock_fetch(path: str):
        if path.startswith("capabilities?"):
            return [{
                "id": "email.send",
        ...
        if path.startswith("capability_services?"):
            return [{
                "service_slug": "resend",
                "credential_modes": ["byo", {"mode": "agent_vault"}, ["rhumb_managed"]],
                "auth_method": "api_key",
                "endpoint_pattern": "POST /emails",
                "cost_per_call": None,
                "cost_currency": "USD",
    ...
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert data["providers"][0]["credential_modes"] == ["byok"]
    assert data["execute_hint"]["credential_modes"] == ["byok"]
...
@pytest.mark.anyio
async def test_resolve_and_credential_modes_not_found_ignore_stale_direct_provider_alias_rows(app):
    async def mock_fetch(path: str):
        if path.startswith("capabilities?"):
            if "id=eq.Resend" in path:
                return []
    ...
    with patch("routes.capabilities.supabase_fetch", new_callable=AsyncMock, side_effect=mock_fetch):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resolve_resp = await client.get("/v1/capabilities/Resend/resolve")
            modes_resp = await client.get("/v1/capabilities/Resend/credential-modes")

    for resp in (resolve_resp, modes_resp):
        assert resp.status_code == 404
        body = resp.json()
        assert body["error"] == "capability_not_found"
        assert body["search_url"] == "/v1/capabilities?search=Resend"
