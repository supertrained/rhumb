...
def test_usage_endpoint_rejects_invalid_days_before_reads(client: TestClient) -> None:
    ...
    with (
        ...
        response = client.get("/v1/telemetry/usage", params={"days": 0})
...
def test_provider_health_rejects_invalid_hours_before_reads() -> None:
    with patch("routes.telemetry.supabase_fetch", new_callable=AsyncMock) as mock_fetch:
        response = TestClient(app).get(
            "/v1/telemetry/provider-health",
            params={"hours": 0},
...
def test_recent_endpoint_rejects_invalid_limit_before_reads(client: TestClient) -> None:
    ...
    with (
        ...
        response = client.get("/v1/telemetry/recent", params={"limit": 0})
...
def test_recent_endpoint_canonicalizes_alternate_provider_aliases_when_context_rows_are_already_canonical(
    client: TestClient,
) -> None:
    row = _execution_row(
        execution_id="exec_recent_error_canonical_contexts",
        provider_used="brave-search-api",
        success=False,
        upstream_status=502,
    )
    row["fallback_attempted"] = True
    row["fallback_provider"] = "people-data-labs"
    row["error_message"] = "brave-search-api failed, then pdl credential lookup failed"
...
def test_recent_endpoint_canonicalizes_same_provider_alias_text_when_context_rows_are_already_canonical(
    ...
    row = _execution_row(
        execution_id="exec_recent_error_same_alias_canonical_context",
        provider_used="brave-search-api",
        success=False,
        upstream_status=502,
    )
