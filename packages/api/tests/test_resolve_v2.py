...
@pytest.mark.anyio
async def test_v2_capabilities_rejects_blank_domain_filter(app):
    with patch("routes.capabilities._cached_fetch", new=AsyncMock()) as mock_fetch:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/v2/capabilities", params={"domain": "   "})
...
async def test_v2_capabilities_rejects_invalid_offset_filter(app):
    with patch("routes.capabilities._cached_fetch", new=AsyncMock()) as mock_fetch:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get("/v2/capabilities", params={"offset": -1})
...
@pytest.mark.anyio
async def test_v2_resolve_issues_route_plan_ids_only_for_issuable_candidates(app):
        ...
        json_body={
            "data": {
                ...
                "providers": [
                    ...
                    {
                        "service_slug": "dry-run-provider",
                        "credential_modes": ["byok"],
                        "auth_method": "api_key",
                        "endpoint_pattern": "POST /dry-run",
                        "safety_state": "dry_run_only",
                    },
                    {
                        "service_slug": "blocked-provider",
                        "credential_modes": ["byok"],
                        "auth_method": "api_key",
                        "endpoint_pattern": "POST /blocked",
                        "safety_state": "blocked_policy",
                        "stop_condition": "policy_denied",
                    },
                ],
    ...
    with (
        patch("routes.resolve_v2._forward_internal", new=AsyncMock(return_value=resolve_resp)),
    ...
    assert resp.status_code == 200
    candidates = resp.json()["data"]["route_candidates"]
    executable, dry_run, blocked = candidates
    assert executable["safety_state"] == "executable"
    assert executable["route_plan_id"].startswith("rhrp1.")
    assert executable["route_plan_expires_at"] == issued_at + 300
    assert dry_run["safety_state"] == "dry_run_only"
    assert dry_run["route_plan_id"].startswith("rhrp1.")
    assert dry_run["route_plan_expires_at"] == issued_at + 300
    assert blocked["safety_state"] == "blocked_policy"
    assert blocked["route_plan_id"] is None
    assert blocked["route_plan_expires_at"] is None

    _, policy_source = resolve_v2_route._merge_effective_policy(None, None)
    _, expected = resolve_v2_route._route_plan_boundary_facts_from_candidate(
        capability_id="search.query",
    ...
    validation = resolve_v2_route.validate_route_plan(
        executable["route_plan_id"],
        expected,
...
@pytest.mark.anyio
async def test_v2_estimate_issues_route_plan_id_for_executable_candidate(app):
    issued_at = 1_800_000_000
    ...
    with (
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get(
                "/v2/capabilities/search.query/execute/estimate",
                params={"credential_mode": "byok", "provider": "brave-search-api"},
            )

    assert resp.status_code == 200
    data = resp.json()["data"]
    candidate = data["route_candidates"][0]
    assert candidate["safety_state"] == "executable"
    assert candidate["route_plan_id"].startswith("rhrp1.")
    assert candidate["route_plan_expires_at"] == issued_at + 300

    _, policy_source = resolve_v2_route._merge_effective_policy(None, None)
    _, expected = resolve_v2_route._route_plan_boundary_facts(
        capability_id="search.query",
        estimate_data=data,
        selected_provider_public="brave-search-api",
        credential_mode="byok",
        parameters={},
        raw_request=_request_with_headers({}),
        agent=None,
...
@pytest.mark.anyio
async def test_v2_estimate_rejects_blank_provider_before_forward(app):
    with patch("routes.resolve_v2._forward_internal", new=AsyncMock()) as mock_forward:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            resp = await client.get(
                "/v2/capabilities/email.send/execute/estimate",
                params={"credential_mode": "byok", "provider": "   "},
                headers={"X-Rhumb-Key": FAKE_RHUMB_KEY},
            )
    ...
    assert body["error"]["detail"] == "Provide a non-empty provider slug or omit the filter."
    mock_forward.assert_not_awaited()
...
async def test_v2_execute_hardens_alias_backed_internal_provider_ids_before_public_v2_shaping(
    ...
    execute_resp = _make_mock_response(
        ...
        json_body={
            "data": {
                "execution_id": "exec_v2_alias_harden",
    ...
    mock_receipt = SimpleNamespace(receipt_id="rcpt_v2_alias_harden")
    mock_attribution = MagicMock()
    mock_attribution.to_response_headers.return_value = {
        "X-Rhumb-Receipt-Id": "rcpt_v2_alias_harden",
        "X-Rhumb-Provider": "brave-search-api",
        "X-Rhumb-Layer": "2",
    }
    mock_attribution.to_rhumb_block.return_value = {
        "provider": {"id": "brave-search-api"},
        "receipt_id": "rcpt_v2_alias_harden",
    }
    mock_billing_stream = SimpleNamespace(emit=MagicMock())
    breaker = SimpleNamespace(state=SimpleNamespace(value="closed"))
    breaker_registry = SimpleNamespace(get=lambda *_args, **_kwargs: breaker)
    ...
    execute_call = mock_forward.await_args_list[1]
    assert execute_call.kwargs["json_body"]["provider"] == "brave-search-api"
...
async def test_v2_execute_canonicalizes_alternate_provider_alias_text_when_structured_fields_are_already_canonical(
    ...
    execute_resp = _make_mock_response(
        ...
        json_body={
            "data": {
                "execution_id": "exec_v2_alt_alias_text_success",
                "provider_used": "brave-search-api",
                "credential_mode": "byo",
                "message": "brave-search-api won after pdl comparison",
                "detail": "Retry brave-search-api if pdl drifts again",
                "upstream_response": {
                    "provider_used": "brave-search-api",
                    "provider_slug": "brave-search-api",
                    "selected_provider": "brave-search-api",
                    "fallback_provider": "brave-search-api",
                    "fallback_providers": ["brave-search-api"],
                    "message": "brave-search-api upstream accepted after pdl retry",
                    "detail": "Retry brave-search-api if pdl drifts again",
                },
                "result": {
                    "provider_slug": "brave-search-api",
                    "selected_provider": "brave-search-api",
                    "fallback_providers": ["brave-search-api"],
                    "message": "brave-search-api result accepted after pdl retry",
                    "detail": "Retry brave-search-api if pdl drifts again",
                },
                "latency_ms": 12.3,
                "overhead_ms": 1.1,
    ...
    mock_receipt = SimpleNamespace(receipt_id="rcpt_v2_alt_alias_text_success")
    mock_attribution = MagicMock()
    mock_attribution.to_response_headers.return_value = {
        "X-Rhumb-Receipt-Id": "rcpt_v2_alt_alias_text_success",
        "X-Rhumb-Provider": "brave-search-api",
        "X-Rhumb-Layer": "2",
    }
    mock_attribution.to_rhumb_block.return_value = {
        "provider": {"id": "brave-search-api"},
        "receipt_id": "rcpt_v2_alt_alias_text_success",
    }
    mock_billing_stream = SimpleNamespace(emit=MagicMock())
    breaker = SimpleNamespace(state=SimpleNamespace(value="closed"))
    breaker_registry = SimpleNamespace(get=lambda *_args, **_kwargs: breaker)
    ...
    data = resp.json()["data"]
    assert data["provider_used"] == "brave-search-api"
    assert data["message"] == "brave-search-api won after people-data-labs comparison"
    assert data["detail"] == "Retry brave-search-api if people-data-labs drifts again"
    assert data["upstream_response"] == {
        "provider_used": "brave-search-api",
        "provider_slug": "brave-search-api",
        "selected_provider": "brave-search-api",
        "fallback_provider": "brave-search-api",
        "fallback_providers": ["brave-search-api"],
        "message": "brave-search-api upstream accepted after people-data-labs retry",
        "detail": "Retry brave-search-api if people-data-labs drifts again",
    }
    assert data["result"] == {
        "provider_slug": "brave-search-api",
        "selected_provider": "brave-search-api",
        "fallback_providers": ["brave-search-api"],
        "message": "brave-search-api result accepted after people-data-labs retry",
        "detail": "Retry brave-search-api if people-data-labs drifts again",
    }
...
async def test_v2_execute_failure_canonicalizes_alternate_provider_alias_text_when_structured_fields_are_already_canonical(
    ...
    execute_resp = _make_mock_response(
        ...
        json_body={
            ...
            "error": {
                "code": "UPSTREAM_ERROR",
                "message": "brave-search-api upstream exploded after pdl comparison",
                "detail": "Retry brave-search-api or choose pdl",
                "requested_provider": "brave-search-api",
                "available_providers": ["brave-search-api"],
                "fallback_provider": "brave-search-api",
            },
        },
    )
    ...
    mock_receipt = SimpleNamespace(receipt_id="rcpt_v2_alt_alias_text_failure")
    mock_attribution = MagicMock()
    mock_attribution.to_response_headers.return_value = {
        "X-Rhumb-Receipt-Id": "rcpt_v2_alt_alias_text_failure",
        "X-Rhumb-Provider": "brave-search-api",
        "X-Rhumb-Layer": "2",
    }
    mock_billing_stream = SimpleNamespace(emit=MagicMock())
...
async def test_v2_execute_canonicalizes_same_provider_alias_text_when_structured_fields_are_already_canonical(
    ...
    execute_resp = _make_mock_response(
        ...
        json_body={
            "data": {
                "execution_id": "exec_v2_same_alias_text_success",
                "provider_used": "brave-search-api",
                "credential_mode": "byo",
                "message": "brave-search-api won after brave-search comparison",
                "detail": "Retry brave-search if brave-search drifts again",
                "upstream_response": {
                    "provider_used": "brave-search-api",
                    "provider_slug": "brave-search-api",
                    "selected_provider": "brave-search-api",
                    "fallback_provider": "brave-search-api",
                    "fallback_providers": ["brave-search-api"],
                    "message": "brave-search-api upstream accepted after brave-search retry",
                    "detail": "Retry brave-search if brave-search drifts again",
                },
                "result": {
                    "provider_slug": "brave-search-api",
                    "selected_provider": "brave-search-api",
                    "fallback_providers": ["brave-search-api"],
                    "message": "brave-search-api result accepted after brave-search retry",
    ...
    mock_receipt = SimpleNamespace(receipt_id="rcpt_v2_same_alias_text_success")
    mock_attribution = MagicMock()
    mock_attribution.to_response_headers.return_value = {
        "X-Rhumb-Receipt-Id": "rcpt_v2_same_alias_text_success",
        "X-Rhumb-Provider": "brave-search-api",
        "X-Rhumb-Layer": "2",
    }
    mock_attribution.to_rhumb_block.return_value = {
        "provider": {"id": "brave-search-api"},
        "receipt_id": "rcpt_v2_same_alias_text_success",
    }
    mock_billing_stream = SimpleNamespace(emit=MagicMock())
    breaker = SimpleNamespace(state=SimpleNamespace(value="closed"))
    breaker_registry = SimpleNamespace(get=lambda *_args, **_kwargs: breaker)
    ...
    data = resp.json()["data"]
    assert data["provider_used"] == "brave-search-api"
    assert data["message"] == "brave-search-api won after brave-search-api comparison"
    assert data["detail"] == "Retry brave-search-api if brave-search-api drifts again"
    assert data["upstream_response"] == {
        "provider_used": "brave-search-api",
        "provider_slug": "brave-search-api",
        "selected_provider": "brave-search-api",
        "fallback_provider": "brave-search-api",
        "fallback_providers": ["brave-search-api"],
        "message": "brave-search-api upstream accepted after brave-search-api retry",
        "detail": "Retry brave-search-api if brave-search-api drifts again",
    }
    assert data["result"] == {
        "provider_slug": "brave-search-api",
        "selected_provider": "brave-search-api",
        "fallback_providers": ["brave-search-api"],
        "message": "brave-search-api result accepted after brave-search-api retry",
        "detail": "Retry brave-search-api later",
    }
    assert "brave-search-api-api" not in str(data)
...
async def test_v2_execute_failure_canonicalizes_same_provider_alias_text_when_structured_fields_are_already_canonical(
    ...
    execute_resp = _make_mock_response(
        ...
        json_body={
            ...
            "error": {
                "code": "UPSTREAM_ERROR",
                "message": "brave-search-api upstream exploded after brave-search comparison",
                "detail": "Retry brave-search later",
                "requested_provider": "brave-search-api",
                "available_providers": ["brave-search-api"],
                "fallback_provider": "brave-search-api",
    ...
    mock_receipt = SimpleNamespace(receipt_id="rcpt_v2_same_alias_text_failure")
    mock_attribution = MagicMock()
    mock_attribution.to_response_headers.return_value = {
        "X-Rhumb-Receipt-Id": "rcpt_v2_same_alias_text_failure",
        "X-Rhumb-Provider": "brave-search-api",
        "X-Rhumb-Layer": "2",
    }
    mock_billing_stream = SimpleNamespace(emit=MagicMock())
    ...
    assert error["available_providers"] == ["brave-search-api"]
    assert error["fallback_provider"] == "brave-search-api"
    assert "brave-search-api-api" not in str(error)
