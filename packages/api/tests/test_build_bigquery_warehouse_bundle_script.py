...
def test_extract_sop_defaults_accepts_impersonation_bundle_note() -> None:
    defaults = build_bigquery_warehouse_bundle._extract_sop_defaults(
        {
            "notesPlain": """
            {
...
def test_build_bundle_supports_service_account_impersonation() -> None:
    args = build_bigquery_warehouse_bundle.build_parser().parse_args(
        [
            "--warehouse-ref",
            "bq_main",
    ...
    bundle = build_bigquery_warehouse_bundle._build_bundle(args, sourced={})

    assert bundle["auth_mode"] == "service_account_impersonation"
    assert bundle["authorized_user_json"]["client_id"] == "client-id"
    assert bundle["service_account_email"] == "svc@proj.iam.gserviceaccount.com"
