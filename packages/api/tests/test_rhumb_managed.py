...
@pytest.mark.anyio
async def test_managed_catalog_endpoint_canonicalizes_alias_copy_in_descriptions(app):
    """Managed catalog copy should not leak runtime aliases in public descriptions."""
...
async def test_managed_executor_google_ai_embeddings_normalize_legacy_model_and_text(monkeypatch):
    ...
    async def mock_fetch(path):
        if "rhumb_managed_capabilities?" in path:
            return [
                {
                    "id": 1000,
                    "capability_id": "ai.embed",
                    "service_slug": "google-ai",
                    "description": "Managed Google AI embeddings",
                    "credential_env_keys": ["RHUMB_CREDENTIAL_GOOGLE_AI_API_KEY"],
                    "default_method": "POST",
                    "default_path": "/v1beta/models/{model}:embedContent",
                    "default_headers": {"Content-Type": "application/json"},
                    "daily_limit_per_agent": None,
                }
            ]
    ...
    with (
        ...
        result = await executor.execute(
            capability_id="ai.embed",
            agent_id="agent_google_embed_test",
            params={
                "model": "models/text-embedding-004",
                "text": "Rhumb routes agents to trusted tools.",
    ...
    assert captured["base_url"] == "https://generativelanguage.googleapis.com"
    assert captured["method"] == "POST"
    assert captured["url"] == "/v1beta/models/gemini-embedding-001:embedContent"
    assert captured["headers"]["x-goog-api-key"] == "gemini_test_secret"
    assert captured["json"] == {
        "content": {"parts": [{"text": "Rhumb routes agents to trusted tools."}]},
