...
def infer_safety_state_and_stop(provider: dict[str, Any]) -> tuple[str, str | None]:
    ...
    if explicit_state in SAFETY_STATES:
        ...
        stop = str(explicit_stop) if explicit_stop in STOP_CONDITIONS else default_stops.get(str(explicit_state))
    ...
    if provider.get("blocked_security") or circuit_state in {"open", "blocked"}:
        return "blocked_security", str(explicit_stop) if explicit_stop in STOP_CONDITIONS else "unverified_artifact"

    if provider.get("experimental_non_default") or provider.get("promotion_state") == "experimental_non_default":
        return "experimental_non_default", str(explicit_stop) if explicit_stop in STOP_CONDITIONS else None
...
def route_candidate_from_provider(
    *,
    ...
    credential_mode = provider.get("credential_mode") or provider.get("preferred_credential_mode") or _first_string(provider.get("credential_modes"))
    ...
    return {
        "route_candidate_id": str(provider.get("route_candidate_id") or stable_route_candidate_id(capability_id, provider_id, rank)),
        "route_id": str(manifest_facts.get("route_id") or provider.get("route_id") or stable_route_id(capability_id, provider_id, substrate)),
        "route_plan_id": provider.get("route_plan_id") if safety_state in {"executable", "dry_run_only"} else None,
        "route_plan_expires_at": provider.get("route_plan_expires_at") if safety_state in {"executable", "dry_run_only"} else None,
        ...
        "manifest_version": manifest_facts.get("manifest_version") or provider.get("manifest_version"),
        "evidence_packet_id": evidence_facts.get("evidence_packet_id") or provider.get("evidence_packet_id"),
        "evidence_packet_digest": evidence_facts.get("evidence_packet_digest") or provider.get("evidence_packet_digest"),
        "evidence_expires_at": evidence_facts.get("evidence_expires_at") or provider.get("evidence_expires_at"),
        "review_status": review_status,
        "safety_state": safety_state,
        "stop_condition": stop_condition,
        "auth_mode": provider.get("auth_method"),
        "credential_mode": credential_mode,
        "required_scopes": manifest_facts["required_scopes"] if "required_scopes" in manifest_facts else provider.get("required_scopes") or [],
        "data_classes": manifest_facts["data_classes"] if "data_classes" in manifest_facts else provider.get("data_classes") or [],
        "side_effect_class": manifest_facts["side_effect_class"] if "side_effect_class" in manifest_facts else provider.get("side_effect_class") or "read",
        ...
        "confirmation_policy": provider.get("confirmation_policy") or ("required" if safety_state == "requires_confirmation" else "none"),
        "sandbox_profile_id": provider.get("sandbox_profile_id"),
        "receipt_support": _enum_value(provider.get("receipt_support"), RECEIPT_SUPPORT_LEVELS, "compact" if provider.get("endpoint_pattern") else "none"),
        "verification_path": provider.get("verification_path"),
        "route_explanation_id": provider.get("route_explanation_id"),
        "alternatives_considered": alternatives_considered,
        "why_selected": "highest_ranked_executable_candidate" if is_selected and safety_state == "executable" else ("selected_candidate" if is_selected else None),
...
def unsupported_route_candidate(capability_id: str, reason: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "stop_condition": "unsupported" if reason == "no_providers_registered" else "missing_manifest",
        "auth_mode": None,
...
def route_candidates_from_resolve_data(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (not isinstance(providers, list) or not providers) and data.get("provider"):
        readiness = data.get("execute_readiness") if isinstance(data.get("execute_readiness"), dict) else {}
        provider: dict[str, Any] = {
    ...
    if not isinstance(providers, list) or not providers:
        ...
        return [unsupported_route_candidate(capability_id, str(recovery_hint.get("reason") or "no_candidate"))]

    execute_hint = data.get("execute_hint") if isinstance(data.get("execute_hint"), dict) else {}
    selected_provider_id = execute_hint.get("preferred_provider") if isinstance(execute_hint, dict) else None
    if selected_provider_id is not None:
        selected_provider_id = str(selected_provider_id)
    alternatives = [
        str(provider.get("provider_id") or provider.get("service_slug") or provider.get("provider") or "unknown")
...
def annotate_resolve_body_with_route_candidates(body: dict[str, Any]) -> dict[str, Any]:
    ...
    annotated_data["route_contract"] = {
        ...
        "index_fact_source": "index_evidence_fixture_and_v1_catalog_compat" if has_index_fixture else "v1_catalog_compat_until_PP-15",
