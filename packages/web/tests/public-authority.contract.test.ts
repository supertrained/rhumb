...
const astroResolveCompare = readFileSync(new URL("../../astro-web/src/pages/resolve/compare.astro", import.meta.url), "utf8");
...
describe("public authority pricing contract", () => {
  ...
  it("keeps the shared resolve mental-model auth surfaces aligned with the live execution rails", () => {
    ...
    expect(astroResolve).toContain('const authorityPages = [');
    expect(astroResolve).toContain('Resolve authority map');
    expect(astroResolve).toContain('href: "/resolve/what-is-resolve"');
    expect(astroResolve).toContain('href: "/resolve/compare"');
    expect(astroResolve).toContain('href: "/resolve/keys"');
    expect(astroPublicTruth).toContain('resolveWhatIsUrl: "https://rhumb.dev/resolve/what-is-resolve"');
    expect(astroPublicTruth).toContain('resolveCompareUrl: "https://rhumb.dev/resolve/compare"');
    expect(astroPublicTruth).toContain('resolveKeysUrl: "https://rhumb.dev/resolve/keys"');
    expect(astroPublicTruth).toContain('resolvePricingUrl: "https://rhumb.dev/resolve/per-call-pricing"');
    expect(astroResolveWhatIs).toContain('What is Resolve?');
    expect(astroResolveWhatIs).toContain('Index ranks. Resolve routes.');
    expect(astroResolveWhatIs).toContain('Not a claim that every indexed service is executable today.');
    expect(astroResolveWhatIs).toContain('Resolve starts before the call.');
    expect(astroResolveCompare).toContain('Resolve is not a connector catalog.');
    expect(astroResolveCompare).toContain('Composio');
    expect(astroResolveCompare).toContain('Arcade');
    expect(astroResolveCompare).toContain('Nango');
    expect(astroResolveCompare).toContain('If the hard question is “which provider should my agent trust and call?”, Rhumb should be in the path.');
    expect(astroResolveKeys).toContain('Keys are a routing constraint, not an implementation detail.');
    expect(astroResolveKeys).toContain('Start with governed API key for repeat Rhumb-managed execution.');
    expect(astroResolveKeys).toContain('Use x402 when zero-signup per-call payment is the point');
    expect(astroResolvePerCallPricing).toContain('Per-call pricing only works if the route is explainable.');
    expect(astroResolvePerCallPricing).toContain('Estimate first. Spend second.');
    expect(astroResolvePerCallPricing).toContain('Cost can change the route. Cost must not rewrite the score.');
    ...
    expect(astroResolveRouting).toContain('candidates[].factors.an_score');
    expect(astroResolveRouting).toContain('candidates[].factors.availability');
    expect(astroResolveRouting).toContain('candidates[].factors.estimated_cost');
    expect(astroResolveRouting).toContain('candidates[].factors.latency');
    expect(astroResolveRouting).toContain('candidates[].factors.credential_mode');
    expect(astroResolveRouting).toContain('candidates[].policy_checks');
    expect(astroResolveRouting).toContain('provider availability / circuit state');
    expect(astroResolveRouting).toContain('latency factor is also derived from that circuit state');
    ...
    expect(astroResolveRouting).not.toContain('Availability and health');
  ...
  it("keeps llms discovery surfaces aligned with live rail-based pricing truth", () => {
    ...
    expect(astroLlmsRoute).toContain("What is Resolve?: ${PUBLIC_TRUTH.resolveWhatIsUrl}");
    expect(astroLlmsRoute).toContain("Resolve comparisons: ${PUBLIC_TRUTH.resolveCompareUrl}");
    expect(astroLlmsRoute).toContain("Key management and credential paths: ${PUBLIC_TRUTH.resolveKeysUrl}");
    expect(astroLlmsRoute).toContain("Per-call pricing explainer: ${PUBLIC_TRUTH.resolvePricingUrl}");
