...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-apify-current-depth4.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-apify-depth4.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Apify depth-4 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Apify depth-4 runtime pass failed; inspect artifact for execute vs direct-control details")
        ...
        publication = _run_publish(
            [
                "--service", "apify",
                "--headline", "Apify: current-depth rerun confirms scrape.extract parity through Rhumb Resolve",
                ...
                "--evidence-title", "Apify current-depth runtime rerun parity check via Rhumb Resolve",
        ...
        payload["apify_depth4_pass"]["evidence_id"] = ((publication.get("evidence") or {}).get("id"))
        payload["apify_depth4_pass"]["review_id"] = ((publication.get("review") or {}).get("id"))
        payload["apify_depth4_pass"]["counts_after"] = await _fetch_public_review_stats("apify")
        _json_dump(artifact_path, payload)
    finally:
