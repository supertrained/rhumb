...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-brave-depth11.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-03-brave-depth11.json"
    org_id = f"org_runtime_review_brave_{stamp.lower()}_{short}"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Brave Search depth-11 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "brave-search", "depth11"],
    )
    ...
    try:
        ...
        comparison = {
            ...
            "top_description_match": (rhumb_top or {}).get("description") == (direct_top or {}).get("description"),
        ...
        verdict = "pass" if (
            ...
            and comparison["top_description_match"]
        ...
        if verdict != "pass":
            raise RuntimeError("Brave Search depth-11 runtime pass failed; inspect artifact for parity details")

        publication = _run_publish(
            [
                "--service", CANONICAL_PUBLISH_SLUG,
                "--headline", "Brave Search: depth-11 runtime review confirms search.query parity through Rhumb Resolve",
                "--summary", "Fresh depth-11 runtime review passed for Brave Search search.query through Rhumb Resolve. Managed and direct executions matched on result count, top result fields, and top-3 URL ordering for the same live query.",
                "--evidence-title", "Brave Search depth-11 runtime review parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-11 runtime review passed for Brave Search search.query through Rhumb Resolve. Managed and direct executions matched on result count, top result fields, and top-3 URL ordering for the same live query.",
