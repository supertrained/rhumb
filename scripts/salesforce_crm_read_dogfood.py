#!/usr/bin/env python3
"""Live hosted proof bundle for the AUD-18 Salesforce CRM read-first rail."""
...
def _first_provider(payload: Any) -> dict[str, Any] | None:
    ...
    for provider in providers:
        if isinstance(provider, dict) and provider.get("service_slug") == "salesforce":
            return provider
    return None
...
def _first_credential_mode(payload: Any) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if not isinstance(providers, list):
        return None, None
    for provider in providers:
        if not isinstance(provider, dict) or provider.get("service_slug") != "salesforce":
            continue
    ...
    results = [
        {
            ...
            "payload_check": None if resolve_ok else "missing_salesforce_provider_or_resolve_handoff",
            "payload": {
        ...
        },
        {
            ...
            "payload_check": None if credential_modes_ok else "missing_salesforce_byok_mode",
...
def _make_describe_success_check(*, crm_ref: str, object_type: str, property_names: tuple[str, ...]) -> PayloadCheck:
    def _check(payload: Any) -> tuple[bool, str | None]:
        ...
        if data.get("provider_used") != "salesforce":
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the hosted Salesforce CRM read-first proof bundle")
...
def _write_artifact(*, args: argparse.Namespace, artifact: dict[str, Any], ok: bool, results: list[dict[str, Any]]) -> int:
    ...
    artifact_path = Path(args.json_out) if args.json_out else ARTIFACTS_DIR / f"aud18-salesforce-crm-hosted-proof-{_now_slug()}.json"
...
def main() -> int:
    ...
    if args.preflight_only or not preflight["configured"]:
        ...
        artifact = {
            ...
            "property_names": list(dict.fromkeys([item.strip() for item in args.property_name if item.strip()])),
    ...
    property_names = tuple(dict.fromkeys([item.strip() for item in args.property_name if item.strip()]))
    ...
    if args.search_filter_property:
        search_payload["filters"] = [
            {
                "property": args.search_filter_property.strip(),
    ...
    checks: list[tuple[str, str, dict[str, Any], int, str | None, PayloadCheck | None, bool]] = [
        ...
        (
            ...
            f"{root}/v1/capabilities/crm.record.get/execute",
            {
                ...
                "object_type": args.object_type,
                "record_id": args.record_id,
                "property_names": [args.denied_property.strip()],
            },
            403,
