...
def render_markdown(contract: CallableContract) -> str:
    ...
    return (
        ...
        "and callable provider slugs. `circuits` is not inventory. `per_service` and "
        "`pools` are this worker's observed windows. They are not inventory. Operators "
        "must not invent slugs.\n"
        "\n"
