...
SENTINEL = "RHUMB_GOOGLE_AI_20260403_DEPTH11"
INPUT_BODY = {
    "model": MODEL,
    "contents": [
        {
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-google-ai-depth11.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-03-google-ai-depth11.json"
    org_id = f"org_runtime_review_google_ai_{stamp.lower()}_{short}"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Google AI depth-11 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "google-ai", "depth11"],
    )
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Google AI depth-11 runtime pass failed; inspect artifact for parity details")

        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Google AI: depth-11 runtime review confirms ai.generate_text parity through Rhumb Resolve",
                "--summary", "Fresh depth-11 runtime review passed for Google AI ai.generate_text through Rhumb Resolve. Managed and direct executions matched on exact output text for the same Gemini 2.5 Flash prompt.",
                "--evidence-title", "Google AI depth-11 runtime review parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-11 runtime review passed for Google AI ai.generate_text through Rhumb Resolve. Managed and direct executions matched on exact output text for the same Gemini 2.5 Flash prompt.",
