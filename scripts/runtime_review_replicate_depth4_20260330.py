...
async def main() -> None:
    ...
    prompt = f"Reply with exactly REPLICATE_CURRENT_DEPTH4_OK_{stamp}"
    model_input = {
        "prompt": prompt,
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-replicate-current-depth4.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-replicate-depth4.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        rate_limit_qpm=100,
        description="Temp review agent for Replicate depth-4 runtime pass",
    ...
    try:
        ...
        comparison = {
            ...
            "output_match": rhumb_output == direct_output == f"REPLICATE_CURRENT_DEPTH4_OK_{stamp}",
        }
        ...
        if verdict != "pass":
            raise RuntimeError("Replicate depth-4 runtime pass failed; inspect artifact for parity details")
