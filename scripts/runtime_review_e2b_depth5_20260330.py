...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-e2b-current-depth5.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-e2b-depth5.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        rate_limit_qpm=100,
        description="Temp review agent for E2B depth-5 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("E2B depth-5 runtime pass failed; inspect artifact for parity details")
