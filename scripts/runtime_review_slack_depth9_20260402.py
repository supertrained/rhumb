#!/usr/bin/env python3
"""Slack current-depth runtime review pass — 2026-04-02.

Slack remains the sole weakest callable provider in the live public audit at depth 8.
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-slack-depth9.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-02-slack-depth9.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Slack depth-9 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Slack depth-9 runtime pass failed; inspect artifact for parity details")
