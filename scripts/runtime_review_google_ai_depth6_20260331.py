...
SENTINEL = "RHUMB_GOOGLE_AI_20260331_DEPTH6"
INPUT_BODY = {
    "model": MODEL,
    "contents": [
        {
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-google-ai-current-depth6.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-31-google-ai-depth6.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Google AI depth-6 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Google AI depth-6 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Google AI: depth-6 freshness rerun confirms ai.generate_text parity through Rhumb Resolve",
                "--summary", "Fresh depth-6 runtime rerun passed for Google AI ai.generate_text through Rhumb Resolve. Managed and direct executions matched on exact output text for the same Gemini 2.5 Flash prompt, lifting the public runtime-backed review stack from depth 5 to depth 6.",
                "--evidence-title", "Google AI depth-6 runtime rerun parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-6 runtime rerun passed for Google AI ai.generate_text through Rhumb Resolve. Managed and direct executions matched on exact output text for the same Gemini 2.5 Flash prompt.",
