...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-tavily-depth11.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-03-tavily-depth11.json"
    org_id = f"org_runtime_review_tavily_{stamp.lower()}_{short}"
    ...
    bootstrap = await ensure_org_billing_bootstrap(
        org_id,
        name=f"runtime-review-tavily-{stamp}",
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Tavily depth-11 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "tavily", "depth11"],
    )
    ...
    try:
        ...
        direct = await _request_json(
            ...
            json_body={
                "api_key": os.environ["RHUMB_CREDENTIAL_TAVILY_API_KEY"],
                "query": QUERY,
                "search_depth": SEARCH_DEPTH,
                "max_results": MAX_RESULTS,
            },
        ...
        rhumb_results = _results(rhumb_upstream)
        direct_results = _results(direct.body)
        rhumb_top = _top_result(rhumb_results)
        direct_top = _top_result(direct_results)
        rhumb_top3_urls = [row.get("url") for row in rhumb_results[:3] if isinstance(row, dict)]
        direct_top3_urls = [row.get("url") for row in direct_results[:3] if isinstance(row, dict)]
        ...
        comparison = {
            ...
            "result_count_match": len(rhumb_results) == len(direct_results) == MAX_RESULTS,
            "top_title_match": (rhumb_top or {}).get("title") == (direct_top or {}).get("title"),
            "top_url_match": (rhumb_top or {}).get("url") == (direct_top or {}).get("url"),
            "top_content_match": (rhumb_top or {}).get("content") == (direct_top or {}).get("content"),
        ...
        verdict = "pass" if (
            ...
            and comparison["top_content_match"]
        ...
        if verdict != "pass":
            raise RuntimeError("Tavily depth-11 runtime pass failed; inspect artifact for parity details")

        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Tavily: depth-11 runtime review confirms search.query parity through Rhumb Resolve",
                "--summary", "Fresh depth-11 runtime review passed for Tavily search.query through Rhumb Resolve. Managed and direct executions matched on result count, top result fields, and top-3 URL ordering for the same live query.",
                "--evidence-title", "Tavily depth-11 runtime review parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-11 runtime review passed for Tavily search.query through Rhumb Resolve. Managed and direct executions matched on result count, top result fields, and top-3 URL ordering for the same live query.",
                "--source-ref", f"runtime-review:tavily:{stamp}",
                "--source-batch-id", f"runtime-review:tavily:{stamp}",
