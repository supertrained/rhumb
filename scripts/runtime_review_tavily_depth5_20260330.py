...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-tavily-current-depth5.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-tavily-depth5.json"
    org_id = f"org_runtime_review_tavily_{stamp.lower()}"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Tavily depth-5 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "tavily"],
    )
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Tavily depth-5 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Tavily: current-depth rerun confirms search.query parity through Rhumb Resolve again",
                "--summary", "Fresh current-depth runtime rerun passed for Tavily search.query through Rhumb Resolve. Managed and direct executions matched on result count, top title, and top URL for the same live search query, lifting Tavily another layer above the callable review floor.",
                "--evidence-title", "Tavily current-depth runtime rerun parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh current-depth runtime rerun passed for Tavily search.query through Rhumb Resolve. Managed and direct executions matched on result count, top title, and top URL for the same live search query.",
