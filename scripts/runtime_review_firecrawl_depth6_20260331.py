#!/usr/bin/env python3
"""Firecrawl current-depth6 runtime review pass — 2026-03-31."""
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-firecrawl-current-depth6.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-31-firecrawl-depth6.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Firecrawl depth-6 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Firecrawl depth-6 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Firecrawl: current-depth rerun confirms scrape.extract parity through Rhumb Resolve at depth 6",
                "--summary", "Fresh current-depth runtime rerun passed for Firecrawl scrape.extract through Rhumb Resolve. Managed and direct executions matched on scrape success, page title, and markdown presence for the same target URL, lifting Firecrawl from claim-safe depth 5 to 6 in the callable review rotation.",
