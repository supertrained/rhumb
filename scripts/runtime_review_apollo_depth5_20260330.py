#!/usr/bin/env python3
"""Apollo current-depth5 runtime review pass - 2026-03-30."""
...
async def _fetch_public_review_stats(service_slug: str) -> dict[str, int | None]:
    result = await _request_json(
        "GET",
        f"{BASE_URL}/services/{service_slug}/reviews",
        timeout=60.0,
    )
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-apollo-current-depth5.json"
    publication_path = (
        REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-apollo-depth5.json"
    )
    org_id = f"org_runtime_review_apollo_{stamp.lower()}"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Apollo depth-5 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "apollo"],
    )
    ...
    try:
        ...
        rhumb_payload = (
            ((execute.body or {}).get("data") or {})
            if isinstance(execute.body, dict)
            else {}
        )
        rhumb_upstream = (
            (rhumb_payload.get("upstream_response") or {})
            if isinstance(rhumb_payload, dict)
            else {}
        )
        ...
        fresh_until = (
            (reviewed_at_dt + timedelta(days=30))
        ...
        comparison["all_fields_match"] = all(field_matches.values())
        ...
        if verdict != "pass":
            raise RuntimeError(
                "Apollo depth-5 runtime pass failed; inspect artifact for parity details"
            )
        ...
        publication = _run_publish(
            [
                ...
                "--headline",
                (
                    "Apollo: current-depth rerun confirms data.enrich_person parity through "
                    "Rhumb Resolve again"
                ),
                "--summary",
                (
                    "Fresh current-depth runtime rerun passed for Apollo data.enrich_person "
                    "through Rhumb Resolve again. Managed and direct executions matched on "
                    "name, title, organization_name, linkedin_url, and email_status for the "
                    "same live email input."
                ),
                "--evidence-title",
                "Apollo current-depth runtime rerun parity check via Rhumb Resolve again",
                "--evidence-summary",
                (
                    "Fresh current-depth runtime rerun passed for Apollo data.enrich_person "
                    "through Rhumb Resolve again. Managed and direct executions matched on "
