...
DIRECT_URL = f"https://api.github.com/users/{TARGET_USERNAME}"
PARITY_FIELDS = [
    "login",
    "id",
    "type",
...
def _get_github_token() -> str | None:
    for env_key in ("RHUMB_CREDENTIAL_GITHUB_TOKEN", "GITHUB_TOKEN", "GH_TOKEN"):
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-github-depth10.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-02-github-depth10.json"
    org_id = f"org_runtime_review_github_{stamp.lower()}_{short}"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for GitHub depth-10 runtime pass",
    ...
    if github_token:
        direct_headers["Authorization"] = f"Bearer {github_token}"
    payload["direct_auth_mode"] = "token" if github_token else "unauthenticated_public_read"
    _json_dump(artifact_path, payload)
    ...
    try:
        ...
        rhumb_fields = {field: rhumb_upstream.get(field) if isinstance(rhumb_upstream, dict) else None for field in PARITY_FIELDS}
        direct_fields = {field: direct.body.get(field) if isinstance(direct.body, dict) else None for field in PARITY_FIELDS}
        field_matches = {field: rhumb_fields.get(field) == direct_fields.get(field) for field in PARITY_FIELDS}
        ...
        if verdict != "pass":
            raise RuntimeError("GitHub depth-10 runtime pass failed; inspect artifact for parity details")

        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "GitHub: depth-10 runtime review confirms social.get_profile parity through Rhumb Resolve",
                "--summary", "Fresh depth-10 runtime review passed for GitHub social.get_profile through Rhumb Resolve. Managed and direct executions matched on identity, company/location metadata, follow counts, and account timestamps for the same live public profile.",
                "--evidence-title", "GitHub depth-10 runtime review parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-10 runtime review passed for GitHub social.get_profile through Rhumb Resolve. Managed and direct executions matched on identity, company/location metadata, follow counts, and account timestamps for the same live public profile.",
