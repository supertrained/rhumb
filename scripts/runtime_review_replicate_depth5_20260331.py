...
async def _poll_prediction(api_token: str, prediction_id: str, *, timeout_seconds: float = 120.0) -> HttpResult:
...
async def main() -> None:
    ...
    prompt = f"Reply with exactly REPLICATE_CURRENT_DEPTH5_OK_{stamp}"
    model_input = {
        "prompt": prompt,
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-replicate-current-depth5.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-31-replicate-depth5.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        rate_limit_qpm=100,
        description="Temp review agent for Replicate depth-5 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "replicate"],
    )
    ...
    access_id = await store.grant_service_access(agent_id, "replicate")
    ...
    try:
        counts_before = await _fetch_public_review_stats("replicate")
        estimate = await _request_json(
            "GET",
            f"{BASE_URL}/capabilities/ai.generate_text/execute/estimate",
            headers=api_headers,
            params={"provider": "replicate", "credential_mode": "rhumb_managed"},
            timeout=60.0,
        )
        ...
        rhumb_final = await _poll_prediction(replicate_token, rhumb_prediction_id) if rhumb_prediction_id else HttpResult(status_code=0, body=None)
        ...
        direct_final = await _poll_prediction(replicate_token, direct_prediction_id) if direct_prediction_id else HttpResult(status_code=0, body=None)
        ...
        comparison = {
            ...
            "output_match": rhumb_output == direct_output == f"REPLICATE_CURRENT_DEPTH5_OK_{stamp}",
        }
        ...
        if verdict != "pass":
            raise RuntimeError("Replicate depth-5 runtime pass failed; inspect artifact for parity details")

        publication = _run_publish(
            [
                "--service", "replicate",
                "--headline", "Replicate: current-depth rerun confirms ai.generate_text parity through Rhumb Resolve",
                "--summary", "Fresh current-depth runtime rerun passed for Replicate ai.generate_text through Rhumb Resolve. Managed and direct executions both created successful predictions and matched on final succeeded state and exact text output for the same pinned model version.",
                "--evidence-title", "Replicate current-depth runtime rerun parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh current-depth runtime rerun passed for Replicate ai.generate_text through Rhumb Resolve. Managed and direct executions both created successful predictions and matched on final succeeded state and exact text output for the same pinned model version.",
        ...
        payload["counts_after"] = await _fetch_public_review_stats("replicate")
        _json_dump(artifact_path, payload)
    finally:
