...
TEXT_FILENAME = "runtime-review-unstructured-depth5.txt"
TEXT_CONTENT = (
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-unstructured-current-depth5.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-unstructured-depth5.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Unstructured depth-5 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "unstructured"],
    )
    ...
    try:
        ...
        rhumb_elements = rhumb_payload.get("upstream_response") if isinstance(rhumb_payload, dict) else []
        direct_elements = direct.body if isinstance(direct.body, list) else []
        ...
        if verdict != "pass":
            raise RuntimeError("Unstructured depth-5 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Unstructured: current-depth rerun confirms document.parse parity through Rhumb Resolve again",
