...
Slack remains the sole weakest callable provider in the live public audit at depth 7.
...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-slack-depth8.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-02-slack-depth8.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Slack depth-8 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Slack depth-8 runtime pass failed; inspect artifact for parity details")
