#!/usr/bin/env python3
"""Run a bounded DC90 Deepgram managed video.subtitle smoke.

The fixture is the same tiny Rhumb-generated WAV used for the media.transcribe
proof.  For video.subtitle we require subtitle-shaped timing output: word-level
start/end timestamps plus the expected fixture words. Secrets are loaded from
...
CAPABILITY_ID = "video.subtitle"
PROVIDER = "deepgram"
INTERFACE = "dc90-deepgram-video-subtitle-smoke-20260428"
...
def request_json(
    ...
    *,
    ...
    merged_headers = {
        ...
        "User-Agent": "rhumb-dc90-deepgram-video-subtitle-smoke/2026-04-28",
    }
    if headers:
...
def request_head(url: str, timeout: float = 20.0) -> tuple[int, dict[str, str]]:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "rhumb-dc90-deepgram-video-subtitle-smoke/2026-04-28"},
        method="HEAD",
    )
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
...
def alternatives(value: Any) -> list[dict[str, Any]]:
...
def collect_timed_words(value: Any) -> list[dict[str, Any]]:
    ...
    for alt in alternatives(value):
        for word in alt.get("words") or []:
            ...
            if isinstance(token, str) and isinstance(start, (int, float)) and isinstance(end, (int, float)):
...
def build_vtt_preview(words: list[dict[str, Any]]) -> str | None:
    if not words:
        return None
...
def compact_step(name: str, status: int, payload: Any) -> dict[str, Any]:
    ...
    if isinstance(payload, dict):
        ...
        if isinstance(upstream, dict):
            ...
            timed_words = collect_timed_words(upstream)
            if transcripts:
                out["transcript_snippets"] = [text[:180] for text in transcripts[:3]]
            if timed_words:
                out["timed_word_count"] = len(timed_words)
                out["timed_word_preview"] = timed_words[:8]
                vtt_preview = build_vtt_preview(timed_words)
                if vtt_preview:
                    out["vtt_preview"] = vtt_preview[:500]
...
def run_smoke(api_key: str, stamp: str, fixture_url: str) -> dict[str, Any]:
    ...
    execute_body = {
        ...
        "interface": INTERFACE,
        "idempotency_key": f"dc90-deepgram-video-subtitle-{stamp.lower()}-{int(time.time() * 1000)}",
        "body": {
            "url": fixture_url,
            "model": "nova-2",
            "smart_format": True,
    ...
    monotonic_timing = all(word["end"] >= word["start"] for word in timed_words)
    vtt_preview = build_vtt_preview(timed_words)
    passed = bool(
        status == 200
        ...
        and len(timed_words) >= 2
        and monotonic_timing
        and vtt_preview
    )
    return {
        "capability_id": CAPABILITY_ID,
        ...
        "timed_word_preview": timed_words[:8],
        "vtt_preview": vtt_preview,
        "passed": passed,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run bounded DC90 Deepgram video.subtitle managed smoke.")
    ...
    artifact = {
        ...
        "summary": {
            ...
            "timed_word_count": result.get("timed_word_count"),
            "fixture_url": args.fixture_url,
        },
    }

    os.makedirs(args.output_dir, exist_ok=True)
    path = os.path.join(args.output_dir, f"dc90-deepgram-video-subtitle-smoke-{stamp}.json")
