...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-brave-current-depth5.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-03-30-brave-depth5.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Brave Search depth-5 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Brave Search depth-5 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", CANONICAL_PUBLISH_SLUG,
                "--headline", "Brave Search: current-depth rerun confirms search.query parity through Rhumb Resolve again",
                "--summary", "Fresh current-depth runtime rerun passed for Brave Search search.query through Rhumb Resolve. Managed and direct executions matched on result count, top title, and top URL for the same live query, lifting the public runtime-backed review stack another layer.",
