...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-twilio-depth7.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-02-twilio-depth7.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Twilio depth-7 runtime pass",
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Twilio depth-7 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", CANONICAL_PUBLISH_SLUG,
                "--headline", "Twilio: depth-7 rerun confirms phone.lookup parity through Rhumb Resolve",
                "--summary", "Fresh depth-7 runtime rerun passed for Twilio phone.lookup through Rhumb Resolve. Managed BYO execution and direct Twilio Lookup v2 returned the same normalized phone record for the same number using Fields=line_type_intelligence.",
                "--evidence-title", "Twilio depth-7 runtime rerun parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-7 runtime rerun passed for Twilio phone.lookup through Rhumb Resolve. Managed BYO execution matched direct Twilio Lookup v2 on phone_number, validity, country metadata, and line_type_intelligence payload.",
