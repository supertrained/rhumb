...
async def main() -> None:
    ...
    artifact_path = REPO_ROOT / f"artifacts/runtime-review-pass-{stamp}-exa-depth10.json"
    publication_path = REPO_ROOT / "artifacts/runtime-review-publication-2026-04-02-exa-depth10.json"
    ...
    agent_id, api_key = await store.register_agent(
        ...
        organization_id=org_id,
        rate_limit_qpm=100,
        description="Temp review agent for Exa depth-10 runtime pass",
        tags=["runtime_review", "phase3", "current_pass", "exa", "depth10"],
    )
    ...
    try:
        ...
        if verdict != "pass":
            raise RuntimeError("Exa depth-10 runtime pass failed; inspect artifact for parity details")

        execution_id = rhumb_payload.get("execution_id") or ""
        publication = _run_publish(
            [
                "--service", SERVICE_SLUG,
                "--headline", "Exa: depth-10 runtime review confirms search.query parity through Rhumb Resolve",
                "--summary", "Fresh depth-10 runtime review passed for Exa search.query through Rhumb Resolve. Managed and direct executions matched on result count, top title, top URL, and top-3 URL ordering for the same live search query.",
                "--evidence-title", "Exa depth-10 runtime review parity check via Rhumb Resolve",
                "--evidence-summary", "Fresh depth-10 runtime review passed for Exa search.query through Rhumb Resolve. Managed and direct executions matched on result count, top title, top URL, and top-3 URL ordering for the same live search query.",
